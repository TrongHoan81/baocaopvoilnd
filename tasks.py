...
def download_report_generator(report_date):
    ...
    try:
        ...
        # 2. Chuẩn bị thư mục Google Drive
        ...
        spreadsheet_summary = google_handler.get_or_create_gsheet(gspread_client, drive_service, f"SanLuong.{date_str_dmy}", month_folder_id)
        ...
        for attempt in range(1, config.MAX_ATTEMPTS + 1):
            ...
            for store_code, store_name in stores_to_process.items():
                ...
                result = api_bh03.download_bh03_report(session, access_token, store_code, report_date)
                summary_row = processor_bh03.process_and_validate_bh03(result, store_name)
                if summary_row:
                    yield f"data:     ✔ Thành công: Dữ liệu hợp lệ.\n\n"
                    successful_summaries.append(summary_row)
                    google_handler.upload_df_to_gsheet(spreadsheet_raw, store_name, result)
                else:
        ...
        if successful_summaries:
            ...
            google_handler.upload_df_to_gsheet(spreadsheet_summary, 'TongHopSanLuong', df_summary)
            yield "data:  -> Đã tải lên file tổng hợp.\n\n"
            try:
                worksheets = spreadsheet_summary.worksheets()
                if len(worksheets) > 1:
                    default_sheet = worksheets[0]
                    if default_sheet.title != 'TongHopSanLuong':
                        spreadsheet_summary.del_worksheet(default_sheet)
                        yield f"data:  -> Đã xóa sheet '{default_sheet.title}' mặc định.\n\n"
            except Exception as e:
                yield f"data:  -> Cảnh báo: Không thể xóa sheet mặc định: {e}\n\n"
